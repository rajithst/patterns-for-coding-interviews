...
class Solution:
    def findUnsortedSubarray(self, nums: List[int]) -> int:
        ...
        """
        find the smallest and largest value which out of order ,because based on the `smallest out of order` value and 
        `largest out of order value`, we may need to re-arrange entire array 
         ex = [1, 3, 2, 0, -1, 7, 10] 
         out of order  range = [2,0,-1] 
         sorting this range will not make entire array sorted ([1, -1, 0, 2, 3, 7, 10]) 
